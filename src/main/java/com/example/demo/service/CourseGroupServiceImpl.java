...
public class CourseGroupServiceImpl implements CourseGroupService {
  ...
  @Override
  @Transactional
  public CourseGroupResponse assign(UUID courseId, CourseGroupAssignRequest request) {
    requireCourse(courseId);
    requireGroup(request.groupId());
    if (courseGroupRepository.findByCourseIdAndGroupId(courseId, request.groupId()).isPresent()) {
      throw new ConflictException(
          "Group " + request.groupId() + " is already assigned to course " + courseId);
    }
  ...
  @Override
  @Transactional
  public void remove(UUID courseId, UUID groupId) {
