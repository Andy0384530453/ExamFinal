...
public class CourseGroupController {
  ...
  public ResponseEntity<CourseGroupResponse> assign(
      ...
      @RequestBody @Valid CourseGroupAssignRequest request) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(courseGroupService.assign(courseId, request, jwt));
  }

  @DeleteMapping("/courses/{courseId}/groups/{groupId}")
  public ResponseEntity<Void> remove(
      @AuthenticationPrincipal Jwt jwt, @PathVariable UUID courseId, @PathVariable UUID groupId) {
    courseGroupService.remove(courseId, groupId, jwt);
